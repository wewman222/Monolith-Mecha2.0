...
public sealed class GunPredictionSystem : SharedGunPredictionSystem
{
    ...
    private void OnShootRequest(RequestShootEvent ev, EntitySessionEventArgs args)
    {
        ...
        if (_gun is GunSystem clientGun)
        {
            clientGun.ShootRequested(ev.Gun, ev.Coordinates, ev.Target, null, args.SenderSession);
        }
        else
        {
            _gun.ShootRequested(GetEntity(ev.Gun), GetCoordinates(ev.Coordinates),
                ev.Target.HasValue ? GetEntity(ev.Target.Value) : null, null, args.SenderSession);
        }
    }
    ...
    private void OnClientProjectileStartCollide(Entity<PredictedProjectileClientComponent> ent, ref StartCollideEvent args)
    {
        ...
        var ev = new PredictedProjectileHitEvent(GetNetEntity(ent.Owner), hit);
    ...
    public override void Update(float frameTime)
    {
        ...
        while (projectiles.MoveNext(out var uid, out var predicted, out var projectile, out var physics))
        {
            ...
            var ev = new PredictedProjectileHitEvent(GetNetEntity(uid), hit);
            RaiseNetworkEvent(ev);
