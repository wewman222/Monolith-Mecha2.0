...
public sealed class GunPredictionSystem : SharedGunPredictionSystem
{
    ...
    private readonly Dictionary<(Guid, NetEntity), EntityUid> _predicted = new();
    ...
    {
        // Convert NetEntity to EntityUid using GetEntity method
        var gunUid = GetEntity(ev.Gun);
        var coordinates = GetCoordinates(ev.Coordinates);
        var target = ev.Target.HasValue ? GetEntity(ev.Target.Value) : (EntityUid?)null;

        _gun.ShootRequested(gunUid, coordinates, target, ev.Shot, args.SenderSession);
    }
    ...
    {
        // Get the projectile's NetEntity
        var netProjectile = ev.Projectile;

        if (!_predicted.TryGetValue((player.UserId, netProjectile), out var projectile))
            return;
